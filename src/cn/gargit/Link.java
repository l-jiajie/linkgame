...
import cn.gargit.board.GameServiceImpl;
...
public class Link extends Activity {
	...
	private void init() {
		Bitmap bitmap = BitmapFactory.decodeResource(this.getResources(), R.drawable.p_1);
		config = new GameConf(8, 9, 2, 10, 100000, bitmap.getWidth(),
		...
		this.gameView.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (event.getAction() == MotionEvent.ACTION_DOWN) {
					Log.i(TAG, "x = " + Float.toString(event.getX()) + "y = " + Float.toString(event.getY()));
	...
	private void gameViewTouchDown(MotionEvent event) // ①
	{
		...
		Log.i(TAG, "x = " + Integer.toString(currentPiece.getCenter().x)
				+"y = " + Integer.toString(currentPiece.getCenter().y));
		// 表示之前没有选中任何一个Piece
